...
\replace{This study applies}{In this study, I apply} machine learning techniques to classify bacterial exotoxins based on their functional types and biological targets.
The methodology follows a structured pipeline that involves data collection, pre-processing, feature extraction, and model training.

First, primary sequences \replace{was}{were} collected and analyzed using descriptive statistics to understand key sequence characteristics. Next, the dataset was preprocessed using redundancy reduction techniques to generate a non-redundant set of sequences.
...
Similarly, bacterial exotoxins were categorized based on their mode of action on how they exert their effect on the host. Bacterial exotoxins were classified into different functional types, including membrane-transducing toxins \add{(Type I)}, pore-forming toxins \add{(Type II)}, intracellular toxins\add{(Type III)}, and toxins that degrade the extracellular matrix \add{(Type IV)}.

\subsection{Dataset Structure and Pre-processing}
The curated dataset was structured in a \replace{CSV}{\textit{csv}} file, which contains bacterial species, their exotoxin types, and their specific targets. This labeled data was used to annotate the input files to build a predictive model for classifying bacterial exotoxins.
\comment{}{usually for file types, only lowercase are used. That is why I changed the CSV to csv. Check the literature before, but I have seen them only in lowercase}
...
The target category consist of vertebrate-targeting exotoxins and non-vertebrate-targeting bacterial exotoxins, with 1,863 and 629 \replace{data points}{sequences}, respectively.
...
This analysis was conducted using custom Python scripts, incorporating key libraries such as Biopython \replace{1.85 }{(version 1.85) }for sequence handling, NumPy \replace{2.2.2}{(version 2.2.2)} for numerical computations, and Matplotlib \replace{3.10.0}{version 3.10.0} for visualization. 
...
This redundancy reduction step is implemented in the \texttt{redundancy-reduction.py} script.

\section{Prediction of 3D Protein Folds}
\label{section:protein_folds}

After applying redundancy reduction to the protein sequences, their three-dimensional (3D) structures were predicted using AlphaFold2 \add{\textbf{reference?}}. The structural confidence of these predicted protein folds is quantified using the per-residue \textit{predicted Local Distance Difference Test} (\textit{pLDDT}) score. The pLDDT scores were used for exploratory purposes and no threshold was set based on pLDDT scores.
...
\remove{In this study, }\add{P}rotein embeddings were generated from both protein sequences and protein folds, utilizing protein language models: ProtT5 and ProstT5.

For protein sequence based protein embedding generation, I employed the ProtT5 language model to generate embeddings for each protein sequence which encodes protein sequence features \cite{elnaggar2021prottrans}. A Google Colab script called embed\_ProtT5.ipynb was used during embedding generation. This script was used in November 2024.

To incorporate structural information, embeddings were also generated using the ProstT5 language model from protein folds, which is trained to encode 3D structural features of proteins \cite{heinzinger2023prostt5}. A Google Colab script called ProstT5.ipynb was used during embedding generation.  This script was used in December 2024.
...
This dimensionality reduction step is implemented in the \texttt{dimensionality-reduction.py} script.
...
\comment{Citations}{Dear Didem, the citations below are not working. Please check their existence in the bib file}
\begin{itemize}
...
All machine learning models are saved in the \texttt{predictors} directory as \comment{pickle files}{Can you add the extension associated with this type of files here, please?}.

\subsection{Additional Approach: Hierarchical Predictor} 
\label{subsection:hierarchical_model}

To address class imbalances and improve classification accuracy, a hierarchical predictor was built. This approach first uses a predictor to predict between group exotoxins into broader type families based on their prevalence. A second-stage classifier is then applied within each group to classify specific exotoxin types with similar datapoints. The architecture of this predictor is shown in Figure~\ref{fig:hier_logic} below.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1.1\textwidth]{images/hierarical_model_drawio.png}
	\caption{The hierarchical predictor architecture for exotoxin classification. The model employs a two-step approach to address class imbalance and improve classification accuracy. In the first stage, a high-level classifier groups exotoxins into broader type families based on their prevalence in the dataset. In \replace{the}{a} second stage, a specialized classifier is applied within each group to refine the classification and distinguish specific exotoxin types that share similar features.}
...
As a baseline method, BLAST (Basic Local Alignment Search Tool) was employed for sequence similarity analysis \cite{Altschul1990}.
...
\replace{P}{For detailed information, p}lease refer to the link for the project's code and the structure of the project's data management plan: \url{https://github.com/didemd/mlclassification_exotoxins}.