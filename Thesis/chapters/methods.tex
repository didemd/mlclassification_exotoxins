...
The methodology follows a structured pipeline (scheme in \ref{fig:methods_workflow}) that involves data collection, preprocessing, feature extraction, and model training.
...
First, \comment{raw protein sequence}{What is raw protein sequence? Usually a raw is the most basic, which in case of proteins will be the amino acid chemical structure. Please describe it as primary sequence or the traditional file for sequences like fasta files} data \comment{is}{Usually methods are described in past tense. It is difficult for me to say here since I am not sure if you are just describing traditional pipelines or what you did in your thesis. Think about it when revising} collected and analyzed using descriptive statistics to understand key sequence characteristics. Then, the dataset is preprocessed by redundancy reduction techniques to derive it to a non-redundant dataset.
Next, protein language models are used to generate numerical embeddings that capture meaningful sequence representations.

Following this, dimensionality reduction techniques are used to refine the embeddings. The processed embeddings are then used to train machine learning models, including conventional classifiers and hierarchical predictors. The machine learning models are evaluated using performance metrics for binary and multiclass classification tasks. 
...
\alert{I think this is not necessary if you keep my reference to the graphic I introduced in the first part of the section.}



\section{Data Collection and Curation}

\alert{Dear Didem, I thought we discussed this already. Yes, you are telling how the data was collected and curated, however the way you are declaring it here is as if you did it all by yourself. Please tell that the labeling was done by me, using the species associated with the toxin and the knowledge of the type of pathogenic association known in the literature. In science we all work and advance because other people have done previous work we use :-) }
Bacterial \add{toxins }protein sequences were collected from public sequence databases, such as UniProt and NCBI\add{, as decribed previously by Krueger et al}. \cite{kruger2024toxin} The dataset was curated \add{and labeled} by selecting proteins from bacterial pathogens based on their host specificity \add{by PD. Dr. Luisa F. Jimenez-Soto}.
...
The \remove{raw }FASTA file stores each exotoxin ID alongside its corresponding protein sequence. To integrate these datasets, the CSV and FASTA files were merged using their IDs. The resulting fully labeled dataset includes bacterial exotoxin sequences along with their respective types and targets. This data preprocessing step is implemented in the \texttt{data-preprocessing.py} script using Python 3.13.1.

Then, a Python script was employed to eliminate sequences annotated as 'partial' or 'fragment' in their FASTA headers, thereby reducing incomplete sequences. This Python script is available as \texttt{data-cleaning.py}.

\alert{I love you put the files doing each task in here! you are making my life much easier and any other reader can go into the file if they want! Very OpenSciency :) }

\subsection{Dataset Composition}
The final dataset contains 2,473 \replace{data points}{sequences}. Type III toxins have 2,148 entries, including both Type III and Type III Control. Type II toxins have 169 entries, including Type II and Type II PFT bacteria. Type I toxins have 42 entries, while Type IV toxins have 28 entries. An additional 104 entries are categorized as Unknown, as they do not belong to any of the four existing classes.
...
This analysis was conducted using custom Python scripts, incorporating key libraries such as Biopython 1.85 for sequence handling, NumPy 2.2.2 for numerical computations, and Matplotlib 3.10.0 for visualization. This Python script can be accessed through \texttt{hydrophobicity-sequence-length.py} script.
...
By analyzing sequence length distributions, we can detect potential biases, such as an overrepresentation of shorter or longer sequences, which may impact model performance.

\subsection{Hydrophobicity Analysis}
\label{subsection:hydrophobicity}

Hydrophobicity affects \replace{their }{protein }structural stability, folding, and interactions with cellular components\alert{ref?}. The average hydrophobicity $H$ of each sequence was computed using the Kyte-Doolittle scale \alert{(ref?)}:
...
\comment{By evaluating sequence length and hydrophobicity distributions, this analysis provides a foundational understanding of the dataset. These characteristics can help identify statistical outliers and reveal potential biases in the dataset.}{I am not sure if this should be here. This sounds more like a discussion. Since I do not know if you are placing the results of this analysis, I cannot tell if this should be here, in the results or discussion. Keep it mind when writing the rest please.}
...
The redundancy reduction workflow was implemented in the redundancy-reduction.py Python script.

\section{Prediction of 3D Protein Folds}
\label{section:protein_folds}

After applying redundancy reduction to the protein sequences, their three-dimensional (3D) structures were predicted using AlphaFold2. The structural confidence of these predicted protein folds is quantified using the per-residue \textit{predicted Local Distance Difference Test} (\textit{pLDDT}) score.
...
\end{enumerate}
\alert{What did you do with the results of this step? Do you have results? was it just exploratory? did you use the result to stablish a threshold?}
...
A stratified random split was performed using the train\_test\_split function from Scikit-Learn \alert{version?}, with a test size of 20\% and a random state of 42 to ensure reproducibility:
...
For protein sequence based protein embedding generation, I employed the ProtT5 language model to generate embeddings for each protein sequence which encodes protein sequence features\cite{elnaggar2021prottrans}. \alert{how? which website? when was it accessed? etc. Methods have to be described in a way that someone can reproduce what you did. Since embeddings are only from outside sources, describe that you use a collab notebook, accessed (date), and follow the intructions by the authors. Same for the ProstT5}

To incorporate structural information, embeddings were also generated using the ProstT5 language model from protein folds, which is trained to encode 3D structural features of proteins \cite{heinzinger2023prostt5}.
...
\alert{Didem, you should add the file names performing the tasks as you did before. For models and their reproducibility, all different algorithms have specific hyperparameter and options that need to be declared.}
\section{Machine Learning Model Training and Evaluation}
\label{section:ml_training_evaluation}

To ensure robust model validation, 5-fold cross-validation was applied, and \comment{Matthews Correlation Coefficient (MCC}{Are you adding the formula somewhere? Then reference it here please.}) was used as a primary metric for model evaluation.
...
\alert{Please add the reference to each of the type of models.}

\subsection{Additional Approach: Hierarchical Predictor}
\label{subsection:hierarchical_model}

To address class imbalances and improve classification accuracy, a hierarchical predictor was implemented. This approach first uses a classifier to group exotoxins into broader type families based on their prevalence. A second-stage classifier is then applied within each group to refine the classification to specific exotoxin types. The architecture of this predictor is depicted in Figure~\ref{fig:hier_logic} below.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1.1\textwidth]{images/hierarical_model_drawio.png}
	\caption{Architecture behind Hierarchical Models}
	\label{fig:hier_logic}
\end{figure}
\alert{Please extend the caption of the figure. Remember, a graph has to be able to stand on its own. First layer? second? third? what do they mean? Very short but to the point to understand the graph right away. For this assume expert knowledge. Since you are explaining it in the text in a more "non-experty" way, everyone can benefit of both approaches.}

\subsection{Additional Approach: BLAST-Based Predictor}
\label{subsection:blast_predictor}

As a baseline method, BLAST (Basic Local Alignment Search Tool) \alert{ref?} was employed for sequence similarity analysis. To evaluate the predictive power of similarity-based classification, a custom BLAST database was constructed using the training sequences. 
...
\alert{Please make sure you are writing different from what we have in publication or in manuscript, or reference to a Kruger et al (in preparation). If not, our paper might be flag for plagiarism when your thesis is in the e-library of the University. I say this because it sound WAY similar to the manuscript Tanja and I are preparing...}

\subsection{Negative Control: Scrambled Protein Sequence Embeddings}
\label{subsection:negative_control_scrambled_embeddings}

To evaluate whether the machine learning models are based on meaningful biological patterns a negative control was used using scrambled protein sequences to generate protein embeddings.
...
\alert{Dear Didem, I am missing here the deployment of the models (pickle?) which file has the info to use your models (you can reference your README.md in the github). When thinking about Material and Methods, think always: what information will I (you) need if I would start today with this project and I need to reproduce it to create something based on this.}
